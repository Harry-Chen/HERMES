...
namespace hermes::impl {
    ...
    int readdir(const char *path, void *buf, fuse_fill_dir_t filler, off_t offset, struct fuse_file_info *fi) {
        ...
        auto fctx = fuse_get_context();
        auto ctx = static_cast<hermes::impl::context *>(fctx->private_data);
        ctx->backend->iterate_directory(pathView,
                                        [&](const std::string_view &fp, const hermes::metadata &metadata) -> void {
                                            const size_t copied = fp.substr(
                                                pathView == "/" ? pathView.size() : pathView.size() + 1,
                                                std::string::npos).copy(name, std::string::npos);
                                            name[copied] = '\0';
